...
def render_portfolio_html(portfolio: dict[str, Any]) -> str:
    ...
    for index, item in enumerate(records):
        ...
        navigation.append(f'''<a class="record-link" href="#{anchor}">
        ...
        record_cards.append(f'''<article class="semantic-card" id="{anchor}" data-semantic="{_text(semantic_type.lower())}">
    ...
    record_html = "".join(record_cards) or '<div class="empty-state"><strong>No records.</strong><span>This dossier does not contain semantic records yet.</span></div>'
    nav_html = "".join(navigation) or '<p class="rail-empty">No records.</p>'
    type_chips = "".join(f'<span class="chip chip-base">{_text(base)}</span>' for base in bases)
    if edges:
        edges_html = "".join(f'<div class="edge-item"><span>↗</span><code>{_text(edge.get("edge_type") or "unknown")}</code></div>' for edge in edges)
    else:
        edges_html = '<p class="rail-empty">No semantic edges yet</p>'
...
<main><div class="pane-heading"><div><p>Portfolio dossier</p><h2>Dot-path data browser</h2></div><p>{len(records)} semantic records</p></div><div id="no-results" class="empty-state"><strong>No matching fields.</strong><span>Try a broader path or value.</span></div>{record_html}</main>
<aside class="provenance"><section class="panel"><h2>Provenance</h2>{execution_html}</section></aside></div>
<script>
function setExpanded(open){{document.querySelectorAll('.tree-node,.execution').forEach(node=>node.open=open)}}
