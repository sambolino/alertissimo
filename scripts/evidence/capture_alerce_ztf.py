#!/usr/bin/env python3
"""Capture representative ALeRCE/ZTF evidence via the official Python client."""
...
BROKER = "alerce"
SURVEY = "ztf"
DEFAULT_OID = "ZTF18abbuksn"
ENV_OID = "ALERCE_ZTF_OID"
SCRIPT = Path(__file__).resolve()


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("output_dir", nargs="?", help="empty output directory (default: /tmp/alerce-ztf-capture-TIMESTAMP)")
    args = parser.parse_args()
    out, captured_at = prepare_output(args.output_dir, "alerce-ztf")
    ...
    manifest["calls"].update({"query_features": {"status": "skipped_due_to_size", "endpoint_label": "query_features"}})
    write_json(out / "capture_manifest.json", manifest)
