...
RA="$(jq -r '(if type=="array" then .[0] else . end) | .objectData.ramean // .ramean // .ra // .meanra' "$OUT/object.json")"
DEC="$(jq -r '(if type=="array" then .[0] else . end) | .objectData.decmean // .decmean // .dec // .meandec' "$OUT/object.json")"
...
FILES=(object lightcurves cone sherlock_object sherlock_position)
