#!/usr/bin/env python3
"""Capture representative ANTARES/ZTF evidence with antares-client."""
from __future__ import annotations
import argparse, os
from pathlib import Path
from client_capture import describe, finish, git_value, package_version, prepare_output, write_json
SCRIPT=Path(__file__).resolve(); SURVEY="ztf"; DEFAULT_OID="ZTF20aafqubg"; ENV_OID="ANTARES_ZTF_OID"

def main():
 parser=argparse.ArgumentParser(description=__doc__); parser.add_argument("output_dir",nargs="?",help="empty output directory (default: /tmp/antares-ztf-capture-TIMESTAMP)"); a=parser.parse_args()
 out,at=prepare_output(a.output_dir,"antares-ztf"); oid=os.environ.get(ENV_OID,DEFAULT_OID)
 try:
 ...
 locus=capture("get_by_ztf_object_id","get_by_ztf_object_id",{"object_id":oid},lambda:getattr(search,"get_by_ztf_object_id")(oid))
 if locus is None: raise SystemExit(f"ERROR: primary object not found: {oid}")
 locus_id=locus.locus_id; manifest["locus_id"]=locus_id
 ...
 for label in ("alerts","catalog_objects","lightcurve"):
  capture(label,f"Locus.{label}",{"locus_id":locus_id},lambda label=label:getattr(locus,label))
