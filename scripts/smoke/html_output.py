...
def _prepare_directory(path: Path) -> None:
    ...
    try:
        ...
        raise HtmlOutputError(f"HTML output path already exists: {path}") from error
...
def write_smoke_html(result, directory: str | Path) -> Path:
    ...
    cards = []
    for step_index, step in enumerate(result.normalized.steps):
        ...
        for execution_index, execution in enumerate(step.executions):
            for portfolio_index, portfolio in enumerate(execution.portfolios):
                filename = (
                    f"step-{step_index:02d}-execution-{execution_index:02d}-"
                    f"portfolio-{portfolio_index:02d}.html"
                )
                write_portfolio_html(portfolio, output_dir / filename)
                ...
                cards.append(
                    ...
                    "</h2><dl>"
                    f"<dt>Execution</dt><dd>{execution_index} · {_h(execution.execution_id)}</dd>"
                    f"<dt>Portfolio</dt><dd>{portfolio_index} · {_h(portfolio.internal_portfolio_id.value)}</dd>"
                    f"<dt>Provenance</dt><dd>{provenance}</dd>"
                    f"<dt>Requested target IDs</dt><dd>{', '.join(_h(x) for x in requested) or 'none'}</dd>"
                    ...
                    f'<p><a href="{_h(filename)}">Open Portfolio dossier</a></p></article>'
                )
    ...
    index.write_text(
        ...
        "<style>body{max-width:70rem;margin:2rem auto;padding:0 1rem;font:16px/1.5 system-ui;color:#162235}"
        "article{border:1px solid #d7e0ea;border-radius:.7rem;padding:1rem;margin:1rem 0}"
        "h1,h2{line-height:1.2}h2{font-size:1.1rem}dl{display:grid;grid-template-columns:12rem 1fr;gap:.35rem}"
        "dt{font-weight:bold}.unavailable{color:#9a5500}</style></head><body>"
        f"<h1>Portfolio dossiers</h1><p>Scenario: {_h(result.name)} · Workflow: {_h(result.workflow.name)}</p>"
