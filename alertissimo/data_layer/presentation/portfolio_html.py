...
def _render_tree(node, prefix, semantic):
    ...
    for key, value in node.items():
        ...
        if isinstance(value, Mapping):
            out.append(
                '<details class="tree-node" open><summary><span class="folder-icon">◇</span>'
                f'<span>{_h(key)}</span><code>{_h(path)}</code></summary><div class="tree-children">'
            )
            if "__value__" in value:
                out.append(_leaf(key, path, value["__value__"], semantic))
            out.append(_render_tree({k:v for k,v in value.items() if k != "__value__"}, path, semantic))
            out.append('</div></details>')
        else:
            out.append(_leaf(key, path, value, semantic))
    return "".join(out)
