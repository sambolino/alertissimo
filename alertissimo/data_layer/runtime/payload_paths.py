...
def resolve_payload_items(
    ...
    *,
    ...
    expansion = path[-2:] if path.endswith(("[]", "{}")) else None
    if (expansion is None or path in ("[]", "{}") or "[]" in path[:-2]
            or "{}" in path[:-2] or any(
    ...
    for root, root_indexes in roots:
        ...
        resolved.extend((value, (*root_indexes, index)) for index, value in enumerate(values))
