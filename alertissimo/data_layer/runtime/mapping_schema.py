...
class InvalidSemanticPathError(MappingSchemaError):
    """Raised with all mapping keys that cannot be composed from the ontology."""
...
def validate_mapping_file(path: str | Path) -> None:
    ...
    invalid_semantic_paths: list[str] = []
    semantic_model = SemanticPathModel.from_ontology()
    for semantic_path, references in mappings.items():
        ...
        if not semantic_model.is_valid(semantic_path):
            invalid_semantic_paths.append(semantic_path)
    ...
    if invalid_semantic_paths:
        formatted = ", ".join(repr(value) for value in invalid_semantic_paths)
    ...
    for semantic_path, raw_transforms in transforms.items():
        ...
        for raw_reference, raw_specification in raw_transforms.items():
            ...
            if "object_key" in specification:
                _nonempty_string(
                    specification["object_key"],
                    f"{path}: transform {semantic_path!r} {raw_reference!r} object_key",
                )
            if "factor" in specification and (
                isinstance(specification["factor"], bool)
                or not isinstance(specification["factor"], (int, float))
            ):
                raise MappingSchemaError(f"{path}: transform factor must be numeric")
