...
TRANSFORM_KEYS = {"type", "map", "default", "note", "skip_null"}
TRANSFORM_TYPES = {
...
def validate_mapping_file(path: str | Path) -> None:
    ...
    for semantic_path, raw_transforms in transforms.items():
        ...
        for raw_reference, raw_specification in raw_transforms.items():
            ...
            if "default" in specification and transform_type != "value_map":
                raise MappingSchemaError(f"{path}: transform default is only valid for value_map")
