...
def test_context_only_derived_points_are_pruned(tmp_path):
    ...
    assert record.fields["magnitude_rate_points"] == (
        {
            "time.mjd": 11.0,
            "identity.source_id": "b",
            "photometry.g.mag.rate": 0.1,
        },
    )
    assert record.fields["color_points"] == (
...
def test_serialized_feature_vectors_decode_skip_empty_and_collect(tmp_path):
    mappings = {
    ...
    transforms = {
        ...
        "lightcurve@fink.feature_vector_points.g.value": {
            "row#g": {"type": "array_decode", "skip_empty": True}
        },
        "lightcurve@fink.feature_vector_points.r.value": {
            "row#r": {"type": "array_decode", "skip_empty": True}
        },
    }
    portfolio = _build(
        tmp_path,
...
def test_non_lightcurve_records_keep_row_cardinality(tmp_path):
    ...
    detections = [
        ...
        if record.semantic_type == "detection@ztf:test"
    ]
    lightcurves = [
        record
        for record in portfolio.records
        if record.semantic_type == "lightcurve@ztf:test"
    ]
