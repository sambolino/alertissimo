from tools.portfolio_browser import _field_tree, render_portfolio_html
...
def test_browser_renders_content_and_escapes_values():
    page = render_portfolio_html({
        ...
        "records": [{"internal_record_id": "record:1", "semantic_type": "summary@ztf:lasair", "internal_source": None, "fields": {"identity.object_id": "<script>alert(1)</script>", "photometry.g.psf.mag": 18.2}}],
        "edges": [],
    })
    ...
    for expected in (
        "Dot-path data browser", "Connection browser", "Expand all", "Collapse all",
