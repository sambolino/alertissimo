        ...
        for portfolio in execution.portfolios:
            for record in portfolio.records:
                if record.semantic_type.split("@", 1)[0] != "summary":
                    continue
                value = record.get("identity.object_id")
...
def test_filter_becomes_runtime_candidate_view_and_downstream_binding_uses_survivors():
    ...
    # A local filter is a semantic view, not a physical execution. It carries the
    ...
    source_a = next(
        ...
        if OBJECT_A in _object_ids(
            type(staged.normalized.steps[2])(
                step_index=2,
                executions=(
                    type(staged.normalized.steps[2].executions[0])(
                        execution_id=staged.normalized.steps[2].executions[0].execution_id,
