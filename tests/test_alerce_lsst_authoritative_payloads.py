...
def test_query_objects_same_oid_yields_classifier_records_without_duplicate_summaries():
    rows=fixture("query_objects"); assert len({r["oid"] for r in rows})==1 and {r["ranking"] for r in rows}=={1}
    p=build("query_objects",rows)
    assert not any(r.semantic_type=="summary@lsst:alerce" for r in p.records)
    assert {r.semantic_type for r in p.records}=={"classification@stamp_classifier_rubin_beta_20260421:alerce","classification@stamp_classifier_rubin_beta:alerce"}
    assert {dict(r.fields)["best.class"] for r in p.records}=={"SN","AGN"}; assert p.edges==()
