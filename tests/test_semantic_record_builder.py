...
def _build_plural(tmp_path, payload, document):
    for definition in document["payloads"].values():
        definition.setdefault("object_partition", {"mode": "single"})
    path = tmp_path / "mappings.yaml"
    path.write_text(yaml.safe_dump(document, sort_keys=False), encoding="utf-8")
    ...
    return build_portfolios_from_execution(
        _execution(payload), mappings_path=path,
        record_id_factory=lambda: InternalRecordId(f"record:{next(ids)}"),
    )
...
def test_skip_null_omits_null_field_when_no_fallback_exists(tmp_path):
    ...
    portfolios = _build_plural(tmp_path, {"value": None}, document)
    assert portfolios == ()
...
def test_skip_null_precedes_jd_arithmetic_and_preserves_real_transform(tmp_path):
    ...
    assert _build_plural(tmp_path, {"value": None}, document) == ()
