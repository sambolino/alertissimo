...
def build_many(endpoint,payload):
    ids=count()
    return build_portfolios_from_execution(ExecutionResult(payload=payload,execution_provenance=InternalExecutionProvenance(internal_execution_id=InternalExecutionId('execution:fixture'),broker='antares',origin='ztf',endpoint=endpoint)),mappings_path=MAPPINGS,record_id_factory=lambda:InternalRecordId(f'record:{next(ids)}'),validate_semantic_model=True)
...
def test_search_cone_and_one_shot_iterator():
    ...
    cone=build_many('cone_search',(x for x in fixture('cone_search.json')))
    expected={row['locus_id'] for row in fixture('cone_search.json')}
    assert len(cone)==len(expected)==4
    assert len({p.internal_portfolio_id for p in cone})==4
    assert all(p.executions[0].internal_execution_id=='execution:fixture' for p in cone)
    assert {records(p,'summary@ztf:antares')[0].fields['identity.antares_locus_id'] for p in cone}==expected
    assert all(len(records(p,'summary@ztf:antares'))==1 for p in cone)
    assert all(not any('separation' in key for r in p.records for key in r.fields) and p.edges==() for p in cone)
