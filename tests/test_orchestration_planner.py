...
def test_explicit_alerce_lsst_forced_photometry(graph):
    plans = plan_step(GetForcedPhotometryStep(sources=[
        Source(broker="alerce", origin="lsst")
    ]), graph)
...
def test_explicit_cone_and_sql_select_only_matching_operations(graph):
    cone = plan_step(ConeSearchStep(
        semantic_type="summary", ra=12, dec=-3, radius=0.1,
        sources=[Source(broker="lasair", origin="ztf")],
    ), graph)
    sql = plan_step(SqlQueryStep(
        semantic_type="summary", query="select * from objects",
        sources=[Source(broker="lasair", origin="ztf")],
    ), graph)
    assert cone[0].endpoint == "cone"
    assert sql[0].endpoint == "query"
    for plan, operation in ((cone[0], "cone_search"), (sql[0], "sql_query")):
        capability = next(item for item in graph.endpoint_capabilities if (
...
def test_multiple_explicit_sources_each_select_one_endpoint(graph):
    ...
    unique_sources = [key for key, values in sorted(by_source.items()) if len(values) == 1]
    assert len(unique_sources) >= 2
    sources = [Source(broker=broker, origin=origin) for broker, origin in unique_sources[:2]]
...
def test_unsupported_deferred_and_local_statuses_are_distinct(graph):
    ...
    for step in (ClassifyStep(), FilterStep(criteria={}), MatchStep(), LightcurveStep()):
        with pytest.raises(PlanningNotApplicableError):
            plan_step(step, graph)


def test_workflow_is_ordered_composition_and_fails_on_local_steps(graph):
    ...
    assert [item.endpoint for item in plan.endpoints] == ["query", "lightcurves"]
    _assert_resolves(plan.endpoints)
    with pytest.raises(PlanningNotApplicableError):
